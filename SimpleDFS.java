...
public class SimpleDFS  {
	...
	IntVar selectVariable(IntVar[] v) {
	    if (v.length != 0) {//* XXX We can just find the one with smallest domain and put it the first
	    	int s = findSmallerDomain(v);
	    	IntVar aux = v[0];
	    	v[0] = v[s];
	    	v[s] = aux; //*/
	    	//orderByDomain(v, 0, v.length-1); // XXX Or, we can order the variables by Domain (Slower)
	...
	private int findSmallerDomain(IntVar[] v) // XXX
	{
		int i = 0;
		IntVar min = v[0];
		for(int j = 1; j < v.length; j++)
		{
			if(v[j].domain.getSize() < min.domain.getSize())
			{
				min = v[j];
				i = j;
			}
	...
	private void orderByDomain(IntVar[] v, int izq, int der) { // XXX Unused, since it is pretty slow
		...
		while(i<j){
			while(v[i].domain.getSize() <= pivote.domain.getSize() && i<j) i++;
			while(v[j].domain.getSize() >pivote.domain.getSize()) j--;
			if (i<j) {                      
				aux = v[i];                  
				v[i] = v[j];
				v[j] = aux;
		...
		v[j]=pivote; 
		if(izq < j-1)
			orderByDomain(v,izq,j-1); 
		if(j+1 < der)
			orderByDomain(v,j+1,der); 
	...
	int selectValue(IntVar v) {
	    return v.max(); // Instade of returning the .min(), we return the .max();
	}
